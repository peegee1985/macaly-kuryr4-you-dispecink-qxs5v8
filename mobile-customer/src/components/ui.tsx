...
export function Screen({
  ...
  contentStyle,
  withBottomTabs = true,
}: PropsWithChildren<{ scroll?: boolean; contentStyle?: StyleProp<ViewStyle>; withBottomTabs?: boolean }>) {
...
export function PageHeader({
  ...
  return (
    <View style={styles.header}>
      {onBack ? (
        <Pressable accessibilityRole="button" accessibilityLabel="Zpět" onPress={onBack} style={styles.headerBack}>
...
export function FormField({
  ...
  style,
  ...props
}: TextInputProps & { label: string; error?: string; icon?: IconName; style?: StyleProp<ViewStyle> }) {
  return (
    <View style={style}>
      <Text style={styles.fieldLabel}>{label}</Text>
      <View style={[styles.inputWrap, multiline && styles.inputWrapMultiline, error && styles.inputWrapError]}>
        {icon ? <Ionicons name={icon} size={18} color={colors.textMuted} style={multiline ? styles.multilineIcon : undefined} /> : null}
        <TextInput
...
export function IconButton({ icon, badge = 0, label, onPress }: { icon: IconName; badge?: number; label: string; onPress: () => void }) {
  return (
    ...
    <View style={[styles.tabBar, { paddingBottom: Math.max(insets.bottom, spacing.sm) }]}>
      {tabs.map((tab) => {
        ...
        return (
          ...
          >
            <View>
              <Ionicons name={selected ? tab.activeIcon : tab.icon} size={tab.key === "new" ? 27 : 22} color={selected || tab.key === "new" ? colors.primary : colors.textMuted} />
              {badge > 0 ? (
            ...
            <Text style={[styles.tabLabel, (selected || tab.key === "new") && styles.tabLabelActive]}>{tab.label}</Text>
          </Pressable>
        );
      })}
    </View>
...
const styles = StyleSheet.create({
  ...
  headerBack: { width: 42, height: 42, borderRadius: radius.md, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, alignItems: "center", justifyContent: "center" },
  headerCopy: { flex: 1 },
  eyebrow: { color: colors.primary, fontWeight: "800", fontSize: 11, letterSpacing: 1.8 },
  title: { color: colors.text, fontWeight: "900", fontSize: 27, letterSpacing: -0.7, marginTop: 2 },
  ...
  button: { minHeight: 48, borderRadius: radius.md, paddingHorizontal: spacing.lg, alignItems: "center", justifyContent: "center", flexDirection: "row", gap: spacing.sm },
  ...
  inputWrapMultiline: { minHeight: 96, alignItems: "flex-start", paddingTop: spacing.md },
  inputWrapError: { borderColor: colors.danger },
  input: { color: colors.text, flex: 1, fontSize: 15, paddingVertical: 0 },
  inputMultiline: { minHeight: 72, lineHeight: 21 },
  multilineIcon: { marginTop: 1 },
  fieldError: { color: "#FCA5A5", fontSize: 11, marginTop: 5 },
  pill: { borderRadius: radius.pill, borderWidth: 1, paddingHorizontal: 10, paddingVertical: 5, flexDirection: "row", alignItems: "center", alignSelf: "flex-start", gap: 6 },
  pillDot: { width: 6, height: 6, borderRadius: 3 },
  pillText: { fontWeight: "700", fontSize: 11 },
  iconButton: { width: 44, height: 44, borderRadius: radius.md, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, alignItems: "center", justifyContent: "center" },
  iconBadge: { position: "absolute", top: -5, right: -5, minWidth: 17, height: 17, paddingHorizontal: 3, borderRadius: 9, backgroundColor: colors.danger, alignItems: "center", justifyContent: "center", borderWidth: 1.5, borderColor: colors.background },
  iconBadgeText: { color: colors.white, fontSize: 8, fontWeight: "900" },
  emptyCard: { alignItems: "center", paddingVertical: spacing.xxl },
  emptyIcon: { width: 56, height: 56, borderRadius: 18, backgroundColor: "rgba(245,158,11,0.12)", alignItems: "center", justifyContent: "center", marginBottom: spacing.md },
  ...
  tabBar: { minHeight: 68, backgroundColor: colors.surface, borderTopColor: colors.border, borderTopWidth: 1, flexDirection: "row", paddingTop: 9 },
