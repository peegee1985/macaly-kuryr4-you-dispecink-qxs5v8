...
import type { ChatConversation, ChatMessage, ChatUser, CustomerUser } from "../types";

type Partner = { _id: string; name: string; role: string };

export function ChatScreen({ user }: { user: CustomerUser }) {
  ...
  return (
    <Screen>
      <PageHeader title="Chat s dispečinkem" subtitle="Rychlá komunikace k vašim zásilkám" />
      {available.length === 0 ? (
        <EmptyState icon="chatbubbles-outline" title="Dispečer není dostupný" message="Jakmile bude aktivní dispečer dostupný, zobrazí se zde." />
      ) : (
          ...
          {available.map((item) => {
            ...
            return (
              <Pressable key={item._id} accessibilityRole="button" onPress={() => setPartner(item)} style={({ pressed }) => [styles.partnerCard, pressed && styles.pressed]}>
                <View style={styles.avatar}><Ionicons name="headset-outline" size={23} color={colors.primary} /></View>
                <View style={styles.partnerCopy}>
                  ...
                  <Text style={styles.partnerRole}>Dispečink Kuryr4You</Text>
...
function Conversation({ user, partner, onBack }: { user: CustomerUser; partner: Partner; onBack: () => void }) {
  ...
  return (
    <KeyboardAvoidingView style={styles.keyboard} behavior={Platform.OS === "ios" ? "padding" : undefined} keyboardVerticalOffset={0}>
      <Screen scroll={false} contentStyle={styles.conversationScreen}>
        <PageHeader title={partner.name} subtitle="Dispečink Kuryr4You" onBack={onBack} />
        <FlatList
          ref={listRef}
          data={messages ?? []}
          ...
          ListEmptyComponent={<View style={styles.emptyConversation}><Ionicons name="chatbubble-ellipses-outline" size={30} color={colors.primary} /><Text style={styles.emptyConversationTitle}>Napište dispečerovi</Text><Text style={styles.emptyConversationText}>Můžete se zeptat na cenu, termín nebo průběh zásilky.</Text></View>}
