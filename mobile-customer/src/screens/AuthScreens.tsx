...
type AccessStep =
  | { type: "login" }
  | { type: "register" }
  | { type: "verify"; email: string; password: string; source: "login" | "register" }
  | { type: "forgot" }
...
export function AccessScreen() {
  ...
  const submitLogin = async () => {
    ...
    try {
      ...
      if (result && !result.signingIn && !result.redirect) {
        setStep({ type: "verify", email: normalizedEmail, password, source: "login" });
        setCode("");
      }
    } catch (err) {
  ...
  const submitRegistration = async () => {
    if (!name.trim() || !email.trim() || !password) {
      setError("Vyplňte jméno, e-mail a heslo.");
      return;
    }
    ...
    try {
      ...
      if (result && !result.signingIn && !result.redirect) {
        setStep({ type: "verify", email: normalizedEmail, password, source: "register" });
        setCode("");
      }
    } catch (err) {
      const message = String((err as { message?: string }).message ?? "").toLowerCase();
      setError(message.includes("already") ? "Účet s tímto e-mailem již existuje." : "Registrace se nezdařila. Zkuste to znovu.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <KeyboardAvoidingView style={styles.keyboard} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <Screen contentStyle={styles.screen} withBottomTabs={false}>
        <View style={styles.brand}>
          ...
          <Text style={styles.brandSub}>ZÁKAZNICKÁ APLIKACE</Text>
        </View>
        ...
        <Card style={styles.formCard}>
          {step.type === "login" ? (
            <>
              ...
              <Text style={styles.help}>Použijte stejný účet jako v zákaznickém portálu.</Text>
              <FormField label="E-mail" icon="mail-outline" value={email} onChangeText={setEmail} placeholder="vas@email.cz" keyboardType="email-address" autoCapitalize="none" autoComplete="email" />
              ...
              <View style={styles.linksRow}>
                <Pressable accessibilityRole="button" onPress={() => changeStep({ type: "forgot" })}><Text style={styles.linkMuted}>Zapomenuté heslo?</Text></Pressable>
                <Pressable accessibilityRole="button" onPress={() => changeStep({ type: "register" })}><Text style={styles.link}>Registrace →</Text></Pressable>
              </View>
            </>
          ) : null}
          ...
          {step.type === "register" ? (
            <>
              <Text style={styles.heading}>Nový zákaznický účet</Text>
              <Text style={styles.help}>Registrace je zdarma. E-mail následně ověříte kódem.</Text>
              <FormField label="Celé jméno" icon="person-outline" value={name} onChangeText={setName} placeholder="Jan Novák" autoCapitalize="words" />
              <FormField label="E-mail" icon="mail-outline" value={email} onChangeText={setEmail} placeholder="vas@email.cz" keyboardType="email-address" autoCapitalize="none" />
              <FormField label="Telefon (volitelný)" icon="call-outline" value={phone} onChangeText={setPhone} placeholder="+420 777 111 222" keyboardType="phone-pad" />
              <FormField label="Heslo" icon="lock-closed-outline" value={password} onChangeText={setPassword} placeholder="Minimálně 8 znaků" secureTextEntry />
              <FormField label="Heslo znovu" icon="shield-checkmark-outline" value={confirmPassword} onChangeText={setConfirmPassword} placeholder="Zopakujte heslo" secureTextEntry onSubmitEditing={() => void submitRegistration()} />
              {error ? <Text style={styles.error}>{error}</Text> : null}
              <AppButton title="Vytvořit účet" icon="person-add-outline" loading={loading} onPress={() => void submitRegistration()} />
              <Pressable accessibilityRole="button" onPress={() => changeStep({ type: "login" })}><Text style={styles.back}>← Už mám účet</Text></Pressable>
            </>
          ) : null}
          ...
          {step.type === "verify" ? (
            <>
              ...
              <FormField label="Ověřovací kód" value={code} onChangeText={(value) => setCode(value.replace(/\D/g, "").slice(0, 6))} placeholder="123456" keyboardType="number-pad" maxLength={6} style={styles.codeField} onSubmitEditing={() => void submitVerification()} />
              {info ? <Text style={styles.info}>{info}</Text> : null}
              {error ? <Text style={styles.error}>{error}</Text> : null}
              ...
              <Pressable accessibilityRole="button" onPress={() => changeStep(step.source === "login" ? { type: "login" } : { type: "register" })}><Text style={styles.back}>← Zpět</Text></Pressable>
            </>
          ) : null}
          ...
          {step.type === "forgot" ? (
            <>
              ...
              <FormField label="E-mail" icon="mail-outline" value={email} onChangeText={setEmail} placeholder="vas@email.cz" keyboardType="email-address" autoCapitalize="none" onSubmitEditing={() => void requestReset()} />
              {error ? <Text style={styles.error}>{error}</Text> : null}
...
export function AccountStateScreen({ state, onSignOut }: { state: "inactive" | "wrong-role"; onSignOut: () => void }) {
  const copy = state === "inactive"
    ? { icon: "time-outline" as const, title: "Účet není aktivní", message: "Pro obnovení přístupu kontaktujte dispečink Kuryr4You." }
    : { icon: "person-outline" as const, title: "Toto není zákaznický účet", message: "Přihlaste se zákaznickým účtem. Aplikace řidiče zůstává samostatná." };
  return (
...
const styles = StyleSheet.create({
  ...
  linksRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingTop: spacing.xs },
  link: { color: colors.primary, fontWeight: "800", fontSize: 13, paddingVertical: spacing.sm },
  linkMuted: { color: colors.textMuted, fontWeight: "600", fontSize: 13, paddingVertical: spacing.sm },
